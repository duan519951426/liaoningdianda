...
const main = ()=>{
    const bannerConfig = window._NEWSBANNER_;
    const [imageBox, clickBox] = [
        document.getElementById("news-banner-imageBox"),
        document.getElementById("news-banner-clickBox")
    ];
    const [classname, speed, unit] = ["thisIndex", 25, "px"];
    const maxIndex = bannerConfig.length - 1,
        maxPosition = -378 * (maxIndex + 1);
    let index = 0,
        animated = false,
        clickLi,
        timer;
    ...
    const main = ()=>{
        ...
        if(index == maxIndex){
            ...
            index++;
            clickLi[index].className = classname;
            animate(imageBox, "left", -378 * index, speed, unit, finish);
        }
    };
    /* 点击事件 */
    function liClick(event){
        ...
        if(e.tagName == "LI" && animated == false){
            ...
            if(cIndex != index){
                ...
                index = cIndex;
                clickLi[index].className = classname;
                animate(imageBox, "left", -378 * index, speed * cha, unit, finish);
            }
        }
    }
    /* 初始化 */
    ...
    const imageDomInit = ()=>{
        const w = maxIndex + 2,
            d = 378;
        imageBox.style.cssText = `width: ${w * d}${unit};left: 0;`;
    ...
    const clickDomInit = ()=>{
        let text = "";
        for(let j = maxIndex; j >= 0; j--){
            text = `<li class="${j === 0 ? classname : ""}" data-index="${j}">${j + 1}</li>${text}`;
        }
